...
const beafForm: React.FC<Props> = ({ type = "create", onClose, current }) => {
    ...
    const createMutation = useMutation({
        mutationFn: beafService.create,
        onSuccess: () => {
            queryClient.removeQueries({ queryKey: ["beafs"] });
            revalidator.revalidate();
            notify.success("beaf created successfully");
            reset();
        },
        onError(error) {
    ...
    const updateMutation = useMutation({
        mutationFn: async (body: Generic) => {
            ...
            return beafService.update(body, current.id);
        },
        onSuccess: () => {
            queryClient.removeQueries({ queryKey: ["beafs", { name, page }] });
            revalidator.revalidate();
            notify.success("beaf updated successfully");
        },
        onError(error) {
            notify.error(JSON.parse(error.message)?.detail);
        },
    ...
    return (
        <React.Fragment>
            <div className="mx-auto w-full px-4 pb-8">
                <div>
                    <h2 className="mt-6 text-xl font-semibold tracking-tight">{type == "create" ? "Create" : "Update"} beaf</h2>
                </div>
                <div className="mt-8">
